...
TOOL_SCHEMA: list[dict[str, Any]] = [
    {
        ...
        "function": {
            ...
            # WHY required+additionalProperties: Groq/Llama tool-call parsing is
            # stricter than OpenAI's. An absent "required" on a no-arg tool, or a
            # missing "additionalProperties" guard, can cause the model to emit
            # malformed function-call syntax that Groq rejects with a 400.
            "parameters": {
                "type": "object",
                "properties": {},
                "required": [],
...
def triage_agent(client, model, ticket: str, max_steps: int = 5) -> Triage:
    ...
    for _ in range(1, max_steps + 1):
        ...
        message = client.chat.completions.create(
            ...
            messages=messages,
            tools=TOOL_SCHEMA,
            # WHY parallel_tool_calls=False: some Groq-hosted models emit
            # malformed output when batching multiple tool calls in one turn.
            # Triage only ever needs one tool at a time, so disabling parallel
            # calls keeps the response format clean and avoids 400 errors.
